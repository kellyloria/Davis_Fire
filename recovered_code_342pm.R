...
ppt_mod <- lmer(TSS~ scale(lag_C_PPT) + scale(tsf) + scale(lag_C_PPT*tsf) + (1|site), data=df_model)
summary(ppt_mod)

ppt_mod.1 <- lmer(TSS~ scale(lag_C_PPT)+ scale(tsf) + (1|site), data=df_model)
summary(ppt_mod.1)

ppt_mod.2 <- lmer(TSS~ scale(lag_C_PPT) + scale(tsf) + scale(flow_filled) + (1|site), data=df_model)
summary(ppt_mod.2)

ppt_mod.3 <- lmer(TSS~ scale(lag_C_PPT) + scale(tsf) + scale(flow_filled*tsf) + (1|site), data=df_model)
summary(ppt_mod.3)

ppt_mod.4 <- lmer(TSS~ scale(lag_C_PPT) + (1|site), data=df_model)
summary(ppt_mod.4)

ppt_mod.5 <- lmer(TSS~ scale(lag_C_PPT) + scale(flow_filled)+ (1|site), data=df_model)
summary(ppt_mod.5)
...
model.sel(null_mod, glob_mod, ppt_mod,  rank = "AICc")
