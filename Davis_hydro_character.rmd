...
davis_data_info <- whatNWISdata(siteNumbers = siteNo_davis)
davis_data_info
```
...
flow_data <- rbind(flow_data_OP, flow_data_WI, flow_data_DA)  
flow_data <- water_year(flow_data)
...
sum_peakQ <- flow_data %>%
  ...
  slice_max(order_by = dischargeCMS, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  select(site, WaterYear, date, jday, dischargeCMS) 

sum_peakQ


sum_peakQ_spr <- flow_data %>%
  group_by(site, WaterYear) %>%
  mutate(jday = yday(date)) %>%
  filter(jday<213) %>% # days before August
  slice_max(order_by = dischargeCMS, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  select(site, WaterYear, date, jday, dischargeCMS) 
...
peak_diff <- sum_peakQ_spr %>%
  filter(site %in% c("ophir", "winters")) %>%   # keep only the two sites
  select(site, WaterYear, jday, dischargeCMS) %>%
  pivot_wider(
    names_from = site,
    values_from = c(jday, dischargeCMS)
  ) %>%
  mutate(
    diff_jday = jday_ophir - jday_winters,
    diff_peakQ = dischargeCMS_ophir - dischargeCMS_winters
  )
...
plot <- 
  sum_peakQ_spr%>%
  ggplot(aes(y = jday, x = WaterYear, shape=site)) +
...
plot <- 
  peak_diff%>%
  ggplot(aes(y = diff_jday, x = WaterYear)) +
    geom_point(alpha = 0.8) +
  geom_line() 
...
plot <- 
  peak_diff%>%
  ggplot(aes(y = diff_peakQ, x = WaterYear)) +
    geom_point(alpha = 0.8) +
  geom_line() 
...
davis_ppt <- read.csv("/Users/kellyloria/Documents/DRI/ERDC_Davis_Fire/ShinyERDC/Processed_data/prism_cat_new.csv")
#View(davis_data)
str(davis_ppt)
...
```{r, echo=FALSE, include=FALSE}
# saveRDS(flow_snow_df1, "./Davis_fire_flow_PPT.rds")

flow_snow_save <-flow_snow_df1%>%
  mutate(date = as.Date(date)) %>%
  filter(!Name == "browns") %>%
  group_by(Name, date) %>%
  summarise(flow = mean(dischargeCMS, na.rm = TRUE)) %>%
  filter(date > as.Date("2023-09-30"))

write_csv(flow_snow_save, "/Users/kellyloria/Documents/DRI/ERDC_Davis_Fire/ShinyERDC/Processed_data/flow_snow_save.csv")
