---
title: 'Epilithic biomass over time'
author: "Kelly Loria"
date: "`r Sys.Date()`"
output:
   html_document:
...
# Chem data
davis_data <- read.csv("/Users/kellyloria/Documents/DRI/ERDC_Davis_Fire/Raw_data/Davis_Rock_Scrape_Biomass1.csv") %>% mutate(date = as.Date(date, format = c("%m/%d/%Y"))) %>%
    ...
    Site == "Browns Sub" ~ "browns_sub")) %>%  filter(date<=as.Date("2025-11-19"))

names(davis_data)
str(davis_data)
```

### Collective site algal biomass 
...
p1 <- davis_data %>%
  ...
  labs(y = "Algal biomass (g cm^-2)", x = NULL) +
  guides(shape = "none") +
  theme_minimal()
...
davis_dat <- davis_data %>%
  ...
  summarise(
    mean_bm_gcm2 = mean(AFDM.g.cm..2, na.rm = TRUE),
    sd_bm = sd(AFDM.g.cm..2, na.rm = TRUE),
    .groups = "drop"
  )
...
### Mean and standard deviation of agal biomass on rocks each reach per visit. 
...
p2 <- davis_dat %>%
  ...
  ggplot(aes(x = date, y = mean_bm_gcm2, color = site_lab, group = site_lab)) +
  geom_point(size = 3) +
  geom_line() +
  geom_errorbar(
    aes(ymin = mean_bm_gcm2 - sd_bm, ymax = mean_bm_gcm2 + sd_bm),
    width = 0.1,
    position = position_dodge(width = 0.2)
  ) +
  scale_color_manual(values = site_lab_colors) +
  labs(
    y ="Algal biomass (g cm^-2)",
    x = NULL) +
  theme_minimal()
...
```{r, warning=F, size = 'small', echo=FALSE, include=FALSE}
# write.csv(davis_dat, "/Users/kellyloria/Documents/DRI/ERDC_Davis_Fire/Processes_data/Algal_biomass_QAQC.csv")
```
