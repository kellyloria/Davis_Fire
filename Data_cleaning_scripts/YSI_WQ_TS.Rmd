...
```{r, warning=F, size = 'small', echo=FALSE, include=FALSE}
# 
davis_data_01 <- read.csv("/Users/kellyloria/Documents/DRI/ERDC_Davis_Fire/Raw_data/YSI_KOR_dat_20260224_processed.csv") %>% 
  mutate(datetime = round_date(as.POSIXct(datetime, format="%Y-%m-%d %H:%M:%S"), unit="5 minutes"),
         date = as.Date(datetime)) %>%
  filter(datetime > as.POSIXct("2025-12-20 11:00:00") ) %>%
    mutate(site_lab = case_when(
    stream == "davis" ~ "davis", 
    stream ==  "ophir creek" ~ "ophir",
    stream ==  "winters creek" ~ "winters_usgs",
...
davis_dat_day1 <- davis_data_01 %>%
  select(-site_lab) %>%
  rename(site_lab = stream) %>%
  group_by(site_lab, date) %>%
  summarise(
    across(3:22,
           list(mean = ~mean(.x, na.rm = TRUE),
...
davis_dat_day1 <- davis_dat_day1[, intersect(names(davis_dat_day), names(davis_dat_day1))]

davis_dat_day1 <- davis_dat_day1 %>%
  select(any_of(names(davis_dat_day)))

```

```{r, warning=F, size = 'small', echo=FALSE, include=FALSE}
davis_dat_day_all <- rbind(davis_dat_day, davis_dat_day1)

# write.csv(davis_dat_day_all, "/Users/kellyloria/Documents/DRI/ERDC_Davis_Fire/Processes_data/ysi_WQ_QAQC_daily1.csv")
