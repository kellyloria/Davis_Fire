...
```{r, echo=FALSE, include =F}
# ggsave(plot = DMC_grid_nuts, filename = paste("/Users/kellyloria/Documents/DRI/ERDC_Davis_Fire/Figures/Nuts_concentrations_boxplot_grid_plot.png",sep=""),width=8,height=7,dpi=300)
...
plot_dmc <- function(df_norm, models_tbl, value_label = "N.C. load", 
  ...
  models_tbl <- models_tbl %>%
    dplyr::ungroup() %>%
    dplyr::mutate(slope_label = paste0("slope = ", round(slope, 2)))
  ...
  ggplot(df_norm, aes(Q_c_norm, load_c_norm)) +
    ...
    geom_text(
      data = models_tbl,
      aes(x = 0.55, y = 0.85, label = slope_label),
...
#       shape = 21, fill = NA, color = "black", stroke = 1.5, size = 3
#     ) +
#     # ## try to add in DMC slopes
#     #   geom_text(
#     #   data = models_tbl,
#     #   aes(x = 1, y = 0, label = slope_label),
#     #   hjust = 1.05, vjust = -0.5,
#     #   size = 4,
#     #   inherit.aes = FALSE
#     # ) +
...
#                          flow_label = NULL,
#                          inference_TSS,
#                          value_label = expression(Norm.~TSS~(mg~L^-1)))
# DMC_TSS_plot
...
DMC_DOC_plot <- plot_dmc(davis_DOC, 
                         ...
                         value_label = expression(Norm.~DOC~(mg~L^-1)))
DMC_DOC_plot
```
...
DMC_TDN_plot <- plot_dmc(davis_TDN, models_TDN, 
                         flow_label = NULL,
                         value_label = expression(Norm.~TDN~(mg~L^-1)))
DMC_TDN_plot
```
...
DMC_Al_plot <- plot_dmc(davis_Al, models_Al,
                        flow_label = NULL, #expression(Normalized~cumulative~water~yield~(m^3~day^-1)),
                       value_label = expression(Norm.~Al~(mg~L^-1)))
DMC_Al_plot
```
...
DMC_As_plot <- plot_dmc(davis_As, models_As, 
                        flow_label = NULL, #expression(Normalized~cumulative~water~yield~(m^3~day^-1)),
                       value_label = expression(Norm.~As~(mg~L^-1)))
DMC_As_plot
```
...
DMC_B_plot <- plot_dmc(davis_B, models_B,
                       flow_label = NULL, #expression(Normalized~cumulative~water~yield~(m^3~day^-1)),
                       value_label = expression(Norm.~B~(mg~L^-1)))
DMC_B_plot
```
...
DMC_Ba_plot <- plot_dmc(davis_Ba, models_Ba, 
                        flow_label = NULL, #expression(Normalized~cumulative~water~yield~(m^3~day^-1)),
                       value_label = expression(Norm.~Ba~(mg~L^-1)))
...
DMC_Ca_plot <- plot_dmc(davis_Ca, models_Ca,
                        flow_label = NULL, #expression(Normalized~cumulative~water~yield~(m^3~day^-1)),
                       value_label = expression(Norm.~Ca~(mg~L^-1)))
DMC_Ca_plot
```
...
DMC_Fe_plot <- plot_dmc(davis_Fe, models_Fe,
                        flow_label = NULL, #expression(Normalized~cumulative~water~yield~(m^3~day^-1)),
                       value_label = expression(Norm.~Fe~(mg~L^-1)))
DMC_Fe_plot
```
...
DMC_Mn_plot <- plot_dmc(davis_Mn, models_Mn, 
                        flow_label = NULL, #expression(Normalized~cumulative~water~yield~(m^3~day^-1)),
                       value_label = expression(Norm.~Mn~(mg~L^-1)))
DMC_Mn_plot
```
...
DMC_Sr_plot <- plot_dmc(davis_Sr, models_Sr, 
                        flow_label = NULL, #expression(Normalized~cumulative~water~yield~(m^3~day^-1)),
                       value_label = expression(Norm.~Sr~(mg~L^-1)))
DMC_Sr_plot
```
...
```{r, echo=FALSE, include =F}
## note for final .md render on git
# output:
#   github_document:
#     toc: true
#     toc_depth: 3
